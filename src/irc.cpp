...
// Copyright (c) 2011-2012 REGA Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #REGATEST0\r");
            Send(hSocket, "WHO #REGATEST0\r");
        } else {
            // randomly join #REGA00-#REGA99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #REGA%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #REGA%02d\r", channel_number).c_str());
        }
