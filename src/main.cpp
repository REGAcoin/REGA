...
uint256 hashGenesisBlock("0x52dc4c1688b419e13c68d355a02088b254b5b8e0d7446867f2c0c652137709e6");
...
const string strMessageMagic = "REGA Signed Message:\n";
...
{
    // REGA: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 265 * COIN;
	
	if (nHeight > 80000*64)
		return nFees;
		
	// Subsidy halving
    nSubsidy >>= (nHeight / 80000);
...
{
    // REGA: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "REGA Genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1474209810;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2161241;
	    ...
	    assert(block.hashMerkleRoot == uint256("0x6ccbbee4af9d59979c96fea60090403f011582209f7313b8e53596b0dca91b85"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd4, 0xca, 0x3f, 0xec };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("REGAMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("REGAMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static REGAMiner(CWallet *pwallet)
{
    printf("REGAMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("REGA-miner");
    ...
    try { loop {
        ...
        printf("Running REGAMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&REGAMiner, pwallet));
}
